  ...
  public sumaryAmount(){
    ...
    if(!!this.transactions){
      for(let i=0 ;i <=this._uw.transactionsSize; i++){
        if(this.transactions[i].type!=undefined && this.transactions[i].type==='one' && this.transactions[i].status==='complete' ){ 
          this.saldo=this.saldo+this.transactions[i].ammount;
        }
        if(this.transactions[i].type!=undefined && this.transactions[i].type==='four' && this.transactions[i].status==='complete' ){ 
          this.saldo=this.saldo-this.transactions[i].ammount;
        } 
        if(this.transactions[i].type!=undefined && this.transactions[i].type==='two' && this.transactions[i].status==='complete' ){ 
          this.saldo=this.saldo-this.transactions[i].ammount;
        } 
        if(this.transactions[i].type!=undefined && this.transactions[i].type==='three' && this.transactions[i].status==='complete' ){ 
          this.saldo=this.saldo-this.transactions[i].ammount;
        }
        if(this.transactions[i].type!=undefined && this.transactions[i].type==='five' && this.transactions[i].status==='complete' ){ 
          this.saldo=this.saldo+this.transactions[i].ammount;
        }
      }
    }
